...
model = Sequential()
model.add(Conv1D(128, 55, activation='relu', input_shape=(check, 1)))
model.add(Conv1D(128, 35, activation='relu'))
...
hist = model.fit(X_train, Y_train, validation_data=(X_val, Y_val), batch_size=350, epochs=500, verbose=2, shuffle=True, callbacks=[checkpointer])
