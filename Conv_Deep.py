...
train = float(sys.argv[1]) #Size of training set in percentage
...
# model.add(MaxPooling1D(3))
# model.add(Dropout(0.5))
# model.add(Conv1D(128, int(sys.argv[2]), activation='relu'))
...
hist = model.fit(X_train, Y_train, validation_data=(X_val, Y_val), batch_size=128, epochs=200, verbose=2, shuffle=True, callbacks=[checkpointer])
